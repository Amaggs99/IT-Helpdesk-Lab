...
\- HR security group existed in Active Directory.

\- Emily Davis (edavis) was a member of the HR security group.

\- Mike Wilson (mwilson) belonged to Sales and should not receive access.
...
Created a confidential document:
...
During testing, permissions were verified from both the server and client computers to ensure security was functioning correctly.



PowerShell was used to validate:
...
\- HR users were granted access.

\- Sales users were denied access.

\- File permissions behaved as expected.
...
\- NTFS permissions determine what users can do once they access a folder.

\- Share permissions control network access to shared folders.

\- Combining Share and NTFS permissions provides layered security.

\- Security groups simplify permission management by assigning access to groups rather than individual users.

\- PowerShell provides a quick method to validate both NTFS and SMB Share permissions.
